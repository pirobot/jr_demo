...
def setup_task_environment(self):
    ...
    rospy.loginfo("BATTERY THRESHOLD " + str(self.low_battery_threshold))
    ...
    self.move_base_timeout = rospy.get_param("~move_base_timeout", 120) #seconds
    ...
    self.waypoints.append(Pose(Point(-0.65, 1.509, 0.000), Quaternion(0, 0, 1, 0)))
    self.waypoints.append(Pose(Point(-1.111, 0.906, 0.000), Quaternion(0.000, 0.000, 0.944, 0.331)))
    self.waypoints.append(Pose(Point(-1.350, 1.715, 0.000), Quaternion(0.000, 0.000, 0.997, 0.071)))
    ...
    self.docking_station_pose = (Pose(Point(0.0, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))
    
    # The robot should go here before engaging the Kobuki auto dock action
    self.pre_dock_pose = (Pose(Point(-0.75, 0.0, 0.0), Quaternion(0.0, 0.0, 0.0, 1.0)))            
    ...
    # Set a marker for the pre dock position
    init_pre_dock_marker(self)
...
def init_pre_dock_marker(self):
    # Define a marker for the charging station
    marker_scale = 0.2
    marker_lifetime = 0 # 0 is forever
    marker_ns = 'waypoints'
    marker_id = 0
    marker_color = {'r': 0.2, 'g': 0.5, 'b': 0.2, 'a': 0.9}
    
    self.pre_dock_marker_pub = rospy.Publisher('pre_dock_marker', Marker, queue_size=5)
    
    self.pre_dock_marker = Marker()
    self.pre_dock_marker.ns = marker_ns
    self.pre_dock_marker.id = marker_id
    self.pre_dock_marker.type = Marker.CYLINDER
    self.pre_dock_marker.action = Marker.ADD
    self.pre_dock_marker.lifetime = rospy.Duration(marker_lifetime)
    self.pre_dock_marker.scale.x = marker_scale
    self.pre_dock_marker.scale.y = marker_scale
    self.pre_dock_marker.scale.z = 0.02
    self.pre_dock_marker.color.r = marker_color['r']
    self.pre_dock_marker.color.g = marker_color['g']
    self.pre_dock_marker.color.b = marker_color['b']
    self.pre_dock_marker.color.a = marker_color['a']
    
    self.pre_dock_marker.header.frame_id = 'odom'
    self.pre_dock_marker.header.stamp = rospy.Time.now()
    self.pre_dock_marker.pose = self.pre_dock_pose